...
@dp.message_handler(IsAdmin(), text="🏦 Редактировать   реквезиты", state="*")
async def product_category_create(message: Message, state: FSMContext):
    await state.finish()

    await state.set_state("here_props")
    await message.answer("<b>🏦 Введите название ваших реквезитов 🏷</b>")

@dp.message_handler(IsAdmin(), state="here_props")
async def props_create(message: Message, state: FSMContext):
    if len(message.text) <= 1000:

        update_storage_props(str(message.text))
        ...
        await message.answer(
            f"Ваши реквизиты\n"
            "➖➖➖➖➖➖➖➖➖➖➖➖\n"
            f"<code>{get_props['name']}</code>",

        )
    else:
        await message.answer("<b>Вы превысили  1000 символов.</b>\n")
