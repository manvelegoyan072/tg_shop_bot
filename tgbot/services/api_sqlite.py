...
# Добавление покупки
def add_purchasex(user_id, user_login, user_name, address, purchase_receipt, purchase_count, purchase_price, purchase_price_one,
    ...
    with sqlite3.connect(PATH_DATABASE) as con:
        ...
        con.execute("INSERT INTO storage_purchases "
                    "(user_id, user_login, user_name,address, purchase_receipt, purchase_count, purchase_price, purchase_price_one, purchase_position_id, "
                    "purchase_position_name, purchase_item, purchase_date, purchase_unix, balance_before, balance_after) "
                    "VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
                    [user_id, user_login, user_name, address, purchase_receipt, purchase_count, purchase_price,
...
def get_storage_props():
    # написать селкт на получение списка реквезитов
    with sqlite3.connect(PATH_DATABASE) as con:
        con.row_factory = dict_factory
        sql = 'SELECT name FROM storage_props'

    return con.execute(sql).fetchall()


def update_storage_props(name):
    with sqlite3.connect(PATH_DATABASE) as con:
        con.row_factory = dict_factory
        sql = f"UPDATE storage_props SET name = \"{name}\" WHERE increment = 1"
...
def create_dbx():
    with sqlite3.connect(PATH_DATABASE) as con:
        ...
        if len(con.execute("PRAGMA table_info(storage_purchases)").fetchall()) == 16:
            ...
            # Создание БД с хранением реквезитов
            if len(con.execute("PRAGMA table_info(storage_props)").fetchall()) == 2:
                print("DB was found(9/9)")
            else:
                con.execute("CREATE TABLE storage_props("
                            "increment INTEGER PRIMARY KEY AUTOINCREMENT,"
                            "name TEXT"
